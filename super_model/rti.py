...
- depth_dpt is SHARED for both mask_depth and global_depth (same spatial map,
  different masks). Separate proj layers differentiate the two representations.
- No self-attention on spatial maps — ViT already performs self-attention on
  these same tokens, and DPT fusion convolutions provide local interaction.
...
class DPTFeatureExtractor(nn.Module):
    ...
    Output: [B, 1024, h, w] fused spatial feature map (4×256 channels).
    """
    ...
    def forward(
        ...
        """Reassemble + fuse multi-layer features.
        ...
        Returns:
            [B, 1024, h, w] — fused spatial feature map
        """
        ...
        f3 = self.fusion_blocks[3](spatial_feats[3])           # Layer 12: no skip
        f2 = self.fusion_blocks[2](spatial_feats[2], skip=f3)  # Layer 9: skip from 12
        f1 = self.fusion_blocks[1](spatial_feats[1], skip=f2)  # Layer 6: skip from 9
        f0 = self.fusion_blocks[0](spatial_feats[0], skip=f1)  # Layer 3: skip from 6
...
class RTE(nn.Module):
    ...
    Architecture:
    - 2 DPT modules: rgb_dpt (RGB path) and depth_dpt (Depth path, shared for dep+gdep)
    - 3 projection layers: proj_rgb, proj_dep, proj_gdep (differentiate pooled features)
    - depth_dpt spatial map is computed ONCE and pooled TWICE (normal + reversed mask)
    """
    ...
    def __init__(self, hidden_dim=1024, vit_dim=768):
        ...
        # NOTE: depth_dpt is SHARED for both mask_depth and global_depth
        
        # Separate projections for each pooled token type
    ...
    def _batch_mask_pool(self, f_spatial, masks, b):
        """Batched mask pooling from 1024-dim spatial map.

        Args:
            f_spatial: [B, 1024, h, w] — fused feature map
            masks: [N_masks, H, W] float tensor (0/1) at original image resolution
            b: batch index

        Returns:
            [N_masks, 1024] — one pooled vector per mask (before projection)
        """
        N = masks.shape[0]
        if N == 0:
    ...
    def forward(
        ...
        # Run DPT reassemble+fusion for each modality path
        rgb_spatial = self.rgb_dpt(rgb_vit_intermediates, h_patches, w_patches)
        # Shared depth spatial map — computed ONCE, pooled TWICE
        ...
        for b in range(B):
            ...
            # Batch pool all masks — RGB
            rgb_pooled = self._batch_mask_pool(rgb_spatial, masks_stacked, b)
            rgb_tokens = self.proj_rgb(rgb_pooled)  # [N, 1024]

            # Batch pool all masks — Depth (normal mask)
            dep_pooled = self._batch_mask_pool(dep_spatial, masks_stacked, b)
            dep_tokens = self.proj_dep(dep_pooled)  # [N, 1024]

            # Batch pool all masks — Global Depth (reversed mask, SAME spatial map)
            gdep_pooled = self._batch_mask_pool(dep_spatial, reversed_masks, b)
            gdep_tokens = self.proj_gdep(gdep_pooled)  # [N, 1024]
